...
/* Close the document log and any file descriptors associated with it.
*/
DocumentLog.prototype.close = function close( on_closed ) {            
...
DocumentLog.prototype.append = function append( doc, on_append_complete ) {

    var that = this;
    
    
    var doc_string = JSON.stringify( doc );

    if ( typeof(on_append_complete) != 'function' ) {
	throw "on_append_complete must be a function";
    }
    var payload_length = doc_string.length  + 8; //8 for the sequence number
	...
	var reference = that.position - buffer.length + 16; //end of last write + 16 to skip the length + sequence number
	on_append_complete( null, that.sequence_number, 
			    reference,
			    doc_string.length );
    }

    fs.write(that.fd, buffer, 0, buffer.length, null, on_write_complete );
};

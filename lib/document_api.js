...
this.mount = function(app, dbm) {
    ...
    //TODO check param handling and output of all of these
    ...
    app.get('/:db/_all_docs', function(req, res) {
        ...
        var on_retrieved_successfully = function on_retrieved_successfully( db ) {
            
            var json = {};
                  
            // TODO: Need to stream this over instead of accumulating all the docs
            var docs = [];
            var emit = function emit( doc ) { docs.push( doc ); };
            var on_complete = function on_complete( err ) { 
                ...
                return res.json( json, status[json.error] || 200 ); 
            };
            db.all_docs( emit, on_complete );    
        };
        _retrieve_db_helper( name, on_retrieved );
    ...
    // get a document
    app.get('/:db/:doc_id/:rev?', function(req, res) {
        var db_name = req.params.db;
        var doc_id = req.params.doc_id;
        var rev = re.params.rev;
        ...
        var on_db_retrieved_successfully = function on_db_retrieved_successfully( db ) {
            ...
            var on_doc_retrieved = function on_doc_retrieved( err, doc ) {
                ...
                if ( err ) { 
                    ...
                    return res.json( { error : 'not_found' }, status['not_found'] ); 
                }
                else {
    ...
    //TODO create or update a document
    app.put('/:db/:doc_id/:child?', function(req, res) {

        var name = req.params.db;
        var doc_id = req.params.doc_id;
        //TODO something is wrong with bodyParse middleware, but for now...

...
*/
PageManager.prototype.get_node_at_sync = function get_node_at( page_number ) {
...
*/
PageManager.prototype.write_node = function write_node( node, on_node_written ) {
	...
	if ( written != buffer.length) {
	    return on_node_written( 'Failed to write the entire buffer' );
	}
	else {
	    var page_number = self.next_page_number;
	    self.next_page_number++;
	    return on_node_written( undefined, node, page_number );
	}
    };
...
var BPlusTree = function BPlusTree( file, page_size ) {
    ...
    var page_number = this.page_manager.get_last_page_number();

    if ( page_number == -1 )  {
	this.root = new Node( NODE_TYPES.ROOT )
	...
	try { 
	    ...
	    if ( node.is_root() ) {
		this.current_root = node;
		break;
	    }
	    
	} Catch ( err ) {
	    //some nodes may be corrupt, log for now
	    console.log( "Corrupt node found at page number " + page_number + " in file " + this.file ".");
	    continue;
	}
...
BPlustree.prototype.insert = function insert( key, value, on_inserted ) {
    ...
    //TODO special case when root node is empty
};

 ...
 */
 ...
 /* Update the value associated with a key the B+Tree.
    callbacks:
        value_transform( old_value )
        on_updated( err, sequence_number )
 */ 
BPlusTree.prototype.update = function update( key, value_transform, on_updated ) {
    ...
    if (this.modification_in_progress) {
        var queued_update = function queued_update() {
                try {
                    that.update( key, value_transform, on_updated );
                }
                catch (err) {
                    on_updated(err);
                }
    ...
    if (this.root_page_number == -1) {
        ...
        this.update_at(this.root_page_number, key, value_transform, on_update_ated)
    }
 ...
 */
 BPlusTree.prototype.update_at = function update_at(page_number, key, value_transform, on_updated) {
    ...
    var on_node_read = function on_node_read(err, node) {
            ...
            if (node.is_leaf) {
                ...
                var new_value = value_transform( node.payloads[index_of_key] );                
                node.payloads[index_of_key] = new_value;
	            ...
	            */
                ...
                that.update_at(child_page_number, key, value_transform, on_recursive_update);
...
BPlusTree.prototype.delete = function _delete( key, on_deleted ) {
     ...
     var on_deleted_from_root = function on_deleted_from_root( err, page_number ) {
     ...
     this.delete_from( this.root_page_number, key, on_deleted_from_root );
};
...
*/
BPlusTree.prototype.delete_from = function delete_from( page_number, key, on_deleted_from ) {
	...
	if ( node.is_leaf ) {
	    ...
	    //see if the key exists using a binary search
	     ...
	     //TODO why is this important again?
	    ...
	    return that.delete_from( child_page_number, key, on_recursive_delete_from );

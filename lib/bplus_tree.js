...
Node.prototype.split = function split() {
    ...
    var middle_index = Math.floor(this.keys.length/2);

    //get elements up to and including the middle_index, + 1 accounts for the array starting at 0
    var keys_left = U(this.keys).first( middle_index + 1);
    var payloads_left = U(this.payloads).first( middle_index + 1);

    //get the rest
    var keys_right = U(this.keys).rest( middle_index  + 1);
    var payloads_right = U(this.payloads).rest( middle_index  + 1);
